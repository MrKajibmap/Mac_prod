...
%macro load_loc_dim;
	...
	proc sql;
		...
		select * from etl_ia.pbo_loc_attributes t1
		inner join casuser.PBO_MAT_LIST t2 on t1.pbo_location_id = t2.pbo_location_id
	...
	proc sql;
		...
		select distinct pbo_location_id, pbo_loc_attr_nm, pbo_loc_attr_value
		from work.temp
	...
	proc sql;
		...
		select distinct pbo_loc_attr_nm, pbo_loc_attr_value
		from work.attr_hier_prep;
	quit;

	proc sql;
		create table work.pbo_lvl4 as
		select distinct a.pbo_location_id, put(a.pbo_location_id, 32.) as location_char, pbo_loc_attr_value as parent_pbo_location_id, 'LEVEL4' as LEVELNAME
		from etl_ia.pbo_loc_hierarchy a
		inner join work.attr_hier_prep c on a.pbo_location_id = c.pbo_location_id
		where pbo_location_lvl = 4 and (valid_from_dttm<=&lmvReportDttm. and valid_to_dttm>=&lmvReportDttm.) and c.pbo_loc_attr_nm = 'COMPANY'
		order by a.pbo_location_id;
	quit;

	proc sql;
		create table work.pbo_lvl3_temp as
		select distinct t1.pbo_location_id as restic, t1.parent_pbo_location_id as pbo_location_id, pbo_loc_attr_value as parent_pbo_location_id
		from work.pbo_lvl4 t1
		inner join work.attr_hier_prep t2 on t1.pbo_location_id = t2.pbo_location_id
	...
	proc sql;
		...
		select distinct pbo_location_id, parent_pbo_location_id, 'LEVEL3' as LEVELNAME
	...
	select distinct a.parent_pbo_location_id as pbo_location_id, '1' as parent_pbo_location_id, 'LEVEL2' as LEVELNAME
	...
	select t1.*, (monotonic()+&max_current_code_id.) as loc_id
	from work.middle_lvl t1
	;

	proc sql;
	create table work.lvl23_id as
	select t1.loc_id as pbo_location_id, coalesce(t2.loc_id, input(t1.parent_pbo_location_id, best8.)) as parent_pbo_location_id,
	t1.levelname
	from work.id t1
	left join work.id t2 on t1.parent_pbo_location_id=t2.pbo_location_id;
	quit;

	proc sql;
	create table work.pbo_lvl1_p as
	select distinct 1 as pbo_location_id, 1 as parent_pbo_location_id, 'LEVEL1' as LEVELNAME
	from work.pbo_lvl2_p 
	;
	quit;
	...
	select distinct t1.restic as pbo_location_id, t2.loc_id as parent_pbo_location_id, 'LEVEL4' as LEVELNAME
	from  work.pbo_lvl3_temp t1
	left join work.id t2 on t1.pbo_location_id=t2.pbo_location_id and t1.parent_pbo_location_id = t2.parent_pbo_location_id
	;
	quit;
	...
	select * from etl_ia.pbo_location
	where valid_from_dttm<=&lmvReportDttm. and valid_to_dttm>=&lmvReportDttm.
	;
	quit;
	...
	select t1.pbo_location_id as MEMBER_ID, coalesce(t3.pbo_location_nm, t2.pbo_location_id) as MEMBER_NM,
	coalesce(t3.pbo_location_nm, t2.pbo_location_id) as MEMBER_DESC, '' as VALID_FROM_DTTM, '' as VALID_TO_DTTM,
	t1.LEVELNAME, 'RUR' as REPORTING_CURRENCY_CD
	from work.hier_list t1
	left join work.id t2 on t1.pbo_location_id = t2.loc_id
	left join work.PBO_location t3 on t1.pbo_location_id = t3.pbo_location_id
	;
	quit;
	...
	data casuser.pbo_attrib;
		set work.pbo_attrib;
	run;
	
	data casuser.pbo_list;
		set work.pbo_list;
	run;
