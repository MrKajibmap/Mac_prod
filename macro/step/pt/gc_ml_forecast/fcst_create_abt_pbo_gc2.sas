...
%macro fcst_create_abt_pbo_gc(
	...
	*/
	...
	%let lmvABTDepthYear = 4;	/* Глубина ABT - кол-во лет в обучающей выборке для VF от начала горизонта прогнозирования */

	/* Временной отрезок [lmvSeasonCalcStartDt ; lmvSeasonCalcEndDt] для расчета сезонности */
	%let lmvSeasonCalcStartDt = %str(date%')%sysfunc(putn(%sysfunc(intnx(day,'05JUL2017'd,0)), yymmdd10.))%str(%');
	...
	%let lmvGCHistStartDt  = '01JAN2018'd;
	%let lmvPBOHistStartDt = '01JAN2018'd;
