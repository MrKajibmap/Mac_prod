...
%let na_history_table = nac.na_calculation_result_new;
...
*/
	...
	*/
	...
	/* Считаем количество конкурентов */
	%include '/opt/sas/mcd_config/macro/step/pt/for_abt_competitors_distance.sas';

	/* Считаем trp конкурентов */
	%include '/opt/sas/mcd_config/macro/step/pt/for_abt_competitors_trp.sas';

	/* Добавляем атрибуты ресторанов, количество конкурентов, trp конкурентов */
	proc fedsql sessref=casauto;
		create table public.na_abt6{options replace=true} as
			select
				...
				t3.n_comp_bk_within_1km,
				t3.n_comp_bk_within_2km,
				t3.n_comp_kfc_within_1km,
				t3.n_comp_kfc_within_2km,
				t4.COMP_TRP_BK,
				t4.COMP_TRP_KFC
			from
			...
			on
				t1.pbo_location_id = t2.pbo_location_id
			left join
				casuser.competitors_distance as t3
			on
				t1.pbo_location_id = t3.pbo_location_id and
				intnx('month',t1.sales_dt, 0, 'b') = t3.month_dt
			left join
				casuser.comp_transposed_ml_expand as t4
			on
				t1.sales_dt = t4.report_dt
		;
	quit;
	...
	proc fedsql sessref=casauto;
		...
		create table public.na_abt14{option replace=true} as
			select
				...
				PRICE_LEVEL_ID, 
				WINDOW_TYPE_ID, 
				n_comp_bk_within_1km,
				n_comp_bk_within_2km,
				n_comp_kfc_within_1km,
				n_comp_kfc_within_2km,
				COMP_TRP_BK,
				COMP_TRP_KFC,
				week, 
				weekday, 
