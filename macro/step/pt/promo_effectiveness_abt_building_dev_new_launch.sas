...
*/
	...
	*/
	...
	/* Загрузим информацию о новых запусках */
	proc import file="/data/MN_CALC/new_launches_add_info.csv"
	    out=work.new_launches_info
	    dbms=csv
		replace;
		delimiter=";";
	run;
	
	/* Уберем пропуски и вытянем id товара */
	proc sql;
		create table work.new_launches_info2 as 
			select
				promo_id,
				promo_nm,
				product_nm,
				input(substr(product_nm, find(product_nm, '[') + 1, find(product_nm, ']') - find(product_nm, '[') - 1), best32.) as product_id,
				(case
					when 'Code Benchmark 1'n = '-' then .
					else  input('Code Benchmark 1'n, best32.)
				end) as benchmark1_product_id,
				(case
					when 'Code Benchmark 2'n = '-' then .
					else  input('Code Benchmark 2'n, best32.) 
				end) as benchmark2_product_id,
				coalesce(calculated benchmark1_product_id, calculated benchmark2_product_id) as benchmark_product_id
			from
				work.new_launches_info
			where
				product_nm is not missing and
				calculated benchmark_product_id is not missing;
	quit;
	
	/* Загрузим таблицу в cas */
	data public.new_launches_info;
		set work.new_launches_info2;
	run;


	/* 	Снова создадим таблицу с промо акциями */
	proc fedsql sessref=casauto;
		create table public.promo_ml0{options replace = true} as 
			select
				t1.promo_id,
				t1.hybrid_promo_id,
				t1.start_dt,
	...
	/* Заменим товары в новых запусках на предшественников */
	proc fedsql sessref=casauto;
		create table public.promo_ml{options replace = true} as 
			select
				...
				t1.option_number,
				t1.promo_mechanics,
				coalesce(t2.benchmark_product_id, t1.product_id) as product_id,
				t1.pbo_location_id
			from
				public.promo_ml0 as t1 
			left join
				public.new_launches_info as t2
			on
	...
	*/
	...
	proc casutil;
		...
		droptable casdata="promo_ml0" incaslib="public" quiet;
