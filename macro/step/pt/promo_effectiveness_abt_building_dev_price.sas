...
%let na_history_table = nac.na_calculation_result_new2;
...
*/
	...
	*/
	...
	/* Формируем каркас для добавления цен */
	proc fedsql sessref=casauto;
		create table public.promo_price_skelet{options replace = true} as 
			select
				t1.promo_id,
				t1.hybrid_promo_id,
				t2.pbo_location_id,
	...
	/* Расшиваем интервалы по дням */
	data public.promo_price_skelet2;
		set public.promo_price_skelet;
		format sales_dt DATE9.;
		do sales_dt=start_dt to end_dt;
			output;
		end;
	...
	/* Объединяем промо цены на прошлое и будущее */
	data casuser.promo_price;
		set mn_dict.price_promo_past(drop=channel_cd) mn_dict.price_promo_future(drop=channel_cd);
	run;
	...
	proc fedsql sessref=casauto;
		select
			nmiss(start_dt),
			nmiss(end_Dt),
			nmiss(pbo_location_id),
			nmiss(product_id),
			nmiss(promo_id),
			nmiss(gross_price_amt)
		from
			casuser.promo_price;
	quit;
	...
	proc fedsql sessref=casauto;
		create table casuser.promo_price_filter{options replace=true} as 
			select
				*
			from
				casuser.promo_price
			where
				start_dt is not missing and
				end_dt is not missing and
				gross_price_amt is not missing
		;
	quit;

	/* Добавляем промо цены */
	proc fedsql sessref=casauto;
		create table public.promo_price_skelet3{options replace = true} as
			select
				t1.*,
				t2.gross_price_amt as promo_price
			from
				public.promo_price_skelet2 as t1
			left join
				casuser.promo_price as t2
			on
	...
	/* Объединяем регулярные цены на прошлое и будущее */
	data casuser.regular_price;
		set mn_dict.price_regular_past(where=(channel_cd='ALL')) mn_dict.price_regular_future;
	run;

	/* Добавляем регулярные цены */
	proc fedsql sessref=casauto;
		create table public.promo_price_skelet4{options replace = true} as
			select
				t1.*,
				t2.gross_price_amt as regular_price
			from
				public.promo_price_skelet3 as t1
			left join
				casuser.regular_price as t2
			on
				t1.pbo_location_id = t2.pbo_location_id and
				t1.product_id = t2.product_id and
				t1.sales_dt <= t2.end_dt and
	...
	/* Добавление регулярных цен для промо товаров */
	
	/* Нерегулярные товары */
	proc sql;
		create table work.irregular_product as
			select distinct
				product_id
			from (
				select 
					product_id,
					input(product_attr_value, best32.) as regular_product_id
				from
					etl_ia.product_attributes
				where
					year(valid_to_dttm) = 5999 and
					product_attr_nm = 'REGULAR_ID' and
					product_attr_value is not missing		
			) as t1
			where
				t1.product_id ^= t1.regular_product_id
		;
	quit;

	select count(1) from public.promo_price_skelet4;
	select count(1) from public.promo_price_skelet3;

	select 
		divide(nmiss(regular_price), count(1)) as regular_pcnt,
		divide(nmiss(promo_price), count(1)) as promo_pcnt 
	from 
		public.promo_price_skelet4
	;

	/* Считаем скиду, разницу в цене */
	

	/* 
		Как-то много миссингов, нужно проверить не баг ли это в коде?	
		- может потому что не убрали закрытые/неоткрытые рестораны 
		- есть ли дубли по ключу?
		- пожет быть после data step появляются пропуски?
		- Сколько пересечений по акциям?
	*/

		
	proc fedsql sessref=casauto;
		select
			divide(nmiss(regular_price), count(1)) as pcnt_regular,
			divide(nmiss(promo_price), count(1)) as pcnt_promo
		from
			public.promo_price_skelet4
		;
	quit;
