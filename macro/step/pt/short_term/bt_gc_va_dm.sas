...
proc fedsql sessref=casauto;
	...
	select 
		...
		, sales_dt
		, GC_FCST as gc
	from MAX_CASL.GC_FORECAST_RESTORED_DEC
	where channel_cd = 'ALL'
		and month(sales_dt) = 12
	;
quit;
...
proc fedsql sessref=casauto;
	...
	select 
		...
		, sales_dt
		, GC_FCST as gc
	from MAX_CASL.GC_FORECAST_RESTORED_JANSH
	where channel_cd = 'ALL'
		and month(sales_dt) = 1
	;
quit;
...
proc fedsql sessref=casauto;
	...
	select 
		...
		, sales_dt
		, GC_FCST as gc
	from MAX_CASL.GC_FORECAST_RESTORED_MAR
	where channel_cd = 'ALL'
		and month(sales_dt) = 3
	;
quit;
...
proc fedsql sessref=casauto;
	...
	select 
		...
		, sales_dt
		, GC_FCST as gc
	from MAX_CASL.GC_FORECAST_RESTORED_MAY
	where channel_cd = 'ALL'
		and month(sales_dt) = 5
	;
quit;
...
data CASUSER.VA_GC_ANALYSIS_APPENDED;
	set 
		casuser.history
		casuser.fcst_dec
		casuser.fcst_jan
...
proc fedsql sessref=casauto;
	create table CASUSER.VA_GC_ANALYSIS {options replace=true} as
	select main.*
	...
	from CASUSER.VA_GC_ANALYSIS_APPENDED as main
...
proc casutil;
	droptable 
		casdata		= "VA_GC_ANALYSIS" 
		incaslib	= "MAX_CASL" 
		quiet         
	;                 
run; 

data MAX_CASL.VA_GC_ANALYSIS (promote=yes);
	set CASUSER.VA_GC_ANALYSIS;
run;
