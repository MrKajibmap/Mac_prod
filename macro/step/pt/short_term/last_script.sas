...
/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
%let ETL_CURRENT_DT	= '01dec2020'd;
%let lmvPostfix 	= A3M1_DEC;
/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
...
select sum(TARGET)/(365*4+1) from MAX_CASL.DM_TRAIN_TRP_GC_&lmvPostfix.  where channel_cd = 'ALL';
quit;

proc fedsql sessref=casauto;
select min(sales_dt) from MAX_CASL.TRAIN_ABT_TRP_GC_&lmvPostfix. where channel_cd = 'ALL';
select max(sales_dt) from MAX_CASL.TRAIN_ABT_TRP_GC_&lmvPostfix. where channel_cd = 'ALL';
select avg(avg_koef_DOY) from MAX_CASL.TRAIN_ABT_TRP_GC_&lmvPostfix.  where channel_cd = 'ALL';
quit;
...
proc fedsql sessref=casauto;
select sum(GC_FCST) from MAX_CASL.GC_FORECAST_RESTORED_&lmvPostfix.  where channel_cd = 'ALL';
select min(sales_dt) from MAX_CASL.GC_FORECAST_RESTORED_&lmvPostfix. where channel_cd = 'ALL';
select max(sales_dt) from MAX_CASL.GC_FORECAST_RESTORED_&lmvPostfix. where channel_cd = 'ALL';
quit;

proc fedsql sessref=casauto;
select sales_dt, count(pbo_location_id) 
from MAX_CASL.GC_FORECAST_RESTORED_&lmvPostfix.  
where channel_cd = 'ALL' and GC_FCST is null
group by sales_dt
;
quit;
